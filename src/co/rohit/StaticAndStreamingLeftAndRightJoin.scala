...
object StaticAndStreamingLeftAndRightJoin extends App {
  ...
  .master("local[*]")
  .appName("StreamingAndStaticLeftAndRightJoin")
  ...
  //schema
   val transactionSchema = StructType(List(StructField("card_id",LongType),
      ...
      //val jointype = "rightouter"  use only when right outer join is streaming othewise it will lead to an error.
     ...
     .format("console")
     .outputMode("update")
     .option("checkpointLocation", "checkpoint-locationSaticandstreamingJoinLeftAndRight")
