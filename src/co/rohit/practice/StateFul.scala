...
object StateFul extends App {
  ...
  sc.set("spark.app.name", "stateful word count")
  ...
  def updatefunc(newValues:Seq[Int],previous:Option[Int]):Option[Int]={
    val newCount = previous.getOrElse(0) + newValues.sum
    Some(newCount)
  }
